...
def score_and_return_on_first_stage(model, tokens, lengths):
    ...
    if mpu.is_pipeline_last_stage():
        ...
        gd.debuginfo(prj="mt", info=f'output_log_probs={infoTensor(output_log_probs)}')
    ...
    with torch.no_grad():
        ...
        gd.debuginfo(prj="mt", info=f'attention_mask={attention_mask}, position_ids={position_ids}')
        ...
        if mpu.is_pipeline_last_stage():
            ...
            gd.debuginfo(prj="mt", info=f'log_probs={log_probs}, '
                                        f'indices={indices}, '
                                        f'output_log_probs={output_log_probs}')
    ...
    gd.debuginfo(prj="mt", info=f'output_log_probs={output_log_probs}')
...
def generate_tokens_probs_and_return_on_first_stage(
    ...
    gd.debuginfo(prj="mt", info=f'forward_step={forward_step}')
    ...
    if mpu.is_pipeline_last_stage():
        ...
        gd.debuginfo(prj="mt", info=f'generated_sequence_lengths={generated_sequence_lengths}')
    
    # Whether we have reached a termination id.
    is_generation_done = torch.zeros(batch_size, dtype=torch.uint8,
                                     device=torch.cuda.current_device())

    gd.debuginfo(prj="mt", info=f'is_generation_done={is_generation_done}')
...
def beam_search_and_return_on_first_stage(model, tokens, lengths, beam_size,
                                          stop_token, num_return_gen, length_penalty,
                                          prevent_newline_after_colon=True):
    gd.debuginfo(prj="mt")
